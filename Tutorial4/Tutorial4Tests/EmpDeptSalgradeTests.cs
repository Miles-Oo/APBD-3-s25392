...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps
            .Join(depts,
                emp => emp.DeptNo,
                dept => dept.DeptNo,
                (emp, dept) => new { emp.EName, dept.DName }) // Projekcja tylko na EName i DName
            .ToList();
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps
            .Where(e => e.Comm.HasValue) // Filtrowanie po nie-nullowych Comm
            .Select(e => new { e.EName, e.Comm }) // Projekcja tylko na EName i Comm
            .ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var avgSalariesPerDept = emps
            .GroupBy(e => e.DeptNo)
            .ToDictionary(g => g.Key, g => g.Average(e => e.Sal));        
        
        var result = emps
            .Where(e => e.Sal > avgSalariesPerDept[e.DeptNo])
            .Select(e => e.EName)
            .ToList();
